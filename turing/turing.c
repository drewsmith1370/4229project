...
GLuint loadTextureFromFile(const char *filepath, int width, int height) {
    ...
    if (!textureData) {
        fprintf(stderr, "Failed to allocate memory for texture data.\n");
        fclose(file);
        return 0;
    }

    // Step 3: Read data from the file
    for (int i = 0; i < texelCount; i += 4) {
        if (fscanf(file, "%f %f %f %f", &textureData[i], &textureData[i + 1], 
                   &textureData[i + 2], &textureData[i + 3]) != 4) {
            fprintf(stderr, "Error reading data from file at texel %d.\n", i / 4);
            free(textureData);
            fclose(file);
            return 0;
        }
...
void initializeTextures() {
    textureID1 = loadTextureFromFile("0.1.txt", 513, 513);
    if (textureID1 == 0) {
        fprintf(stderr, "Failed to load texture 0.1.txt.\n");
    }

    textureID2 = loadTextureFromFile("output-hexagons.txt", 169, 169);
    if (textureID2 == 0) {
        fprintf(stderr, "Failed to load texture 0.04.txt.\n");
    }
...
void display() {
   ...
   if(floor(progTime/3) > timecheck || progTime == 0)
   {
      if(progTime != 0)
      {
         timecheck++;
